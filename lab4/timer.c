...
uint32_t timer_count = 0; //initializing timer_count to 0
...
int (timer_set_frequency)(uint8_t timer, uint32_t freq) {
	...
	uint16_t div = TIMER_FREQ / freq; //getting timer_count value (div)
	uint8_t LSB, MSB;
...
void (timer_int_handler)() {
	timer_count++; //incrementing timer_count (at each interrupt)
}
