...
int mouse_enable_data_report() {
	...
	do {
		...
		if (sys_outb(IN_BUF, ENABLE_DATA) != OK)					//Enables data reporting
			return 1; 
...
void (mouse_ih)() {
	...
	if ((uint8_t)stat & (OBF | AUX)) {							//Checks if Out_Buffer is full and that data comes from the mouse
		if (sys_inb(OUT_BUF, &data) != OK) { 				    //Tries to read data inside Out_Buffer
			data = 0;											//Resets data
			return;
		}
...
int mouse_data_handler(){
	if (Nbyte == 0 && !((uint8_t)data & THIRD))							//Checks if bit 3 is set to 1
		return 1;

	pp.bytes[Nbyte] = (uint8_t)data;									//Passes data-bytes to struct packet pp

	if (Nbyte == 0)														//Parses fisrt byte
	{
		pp.rb = ((uint8_t)data & RB);
		pp.mb = ((uint8_t)data & MB);
		pp.lb = ((uint8_t)data & LB);
		x_sign = ((uint8_t)data & X_SIGN);
		y_sign = ((uint8_t)data & Y_SIGN);
		pp.x_ov = ((uint8_t)data & X_OV);
		pp.y_ov = ((uint8_t)data & Y_OV);
	}

	if (Nbyte == 1)														//Checks signal of x
	{
		if (x_sign)
			pp.delta_x = SIGN_EXTEND | (uint16_t)data;
	...
	{
		if (y_sign)
			pp.delta_y = SIGN_EXTEND | (uint16_t)data;
...
int mouse_disable_data_report(){
	...
	do {
		...
		if (sys_outb(IN_BUF, DISABLE_DATA) != OK)					//Disables data reporting
			return 1; //failed
...
int read_data(){
	...
	do {
		...
		if (sys_outb(IN_BUF, READ_DATA) != OK)						//Reads data
			return 1; //failed
